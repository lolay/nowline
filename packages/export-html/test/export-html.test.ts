...
async function svgFor(source: string): Promise<{ inputs: Awaited<ReturnType<typeof buildExportInputs>>; svg: string }> {
...
describe('exportHtml', () => {
    ...
    it('respects an explicit title', async () => {
        const { inputs, svg } = await svgFor(MINIMAL_FIXTURE);
        const html = await exportHtml(inputs, svg, { title: 'Custom Title', generator: STABLE_GENERATOR });
        expect(html).toContain('<title>Custom Title</title>');
    });

    it('escapes HTML special characters in the title', async () => {
        const { inputs, svg } = await svgFor(MINIMAL_FIXTURE);
        const html = await exportHtml(inputs, svg, { title: 'A & B <c>', generator: STABLE_GENERATOR });
