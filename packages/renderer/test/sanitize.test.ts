...
describe('sanitizeSvg', () => {
    it('keeps benign svg content', () => {
        const input = '<svg xmlns="http://www.w3.org/2000/svg"><g><rect x="1" y="2" width="3" height="4"/></g></svg>';
    ...
    it('drops <script>', () => {
        ...
        const input = '<svg><script>alert(1)</script><rect x="0" y="0" width="4" height="4"/></svg>';
    ...
    it('allows fragment-only hrefs and rewrites them', () => {
        const input = '<svg><defs><g id="logo"><rect x="0" y="0" width="4" height="4"/></g></defs><use href="#logo"/></svg>';
    ...
    it('drops inline style attributes', () => {
        const input = '<svg><rect style="fill:url(http://evil)" x="0" y="0" width="4" height="4"/></svg>';
        const out = sanitizeSvg(input);
        expect(out).not.toContain('style=');
    });

    it('rewrites url(#id) references inside fill', () => {
        const input = '<svg><defs><linearGradient id="g1"><stop offset="0" stop-color="#f00"/></linearGradient></defs><rect fill="url(#g1)" x="0" y="0" width="4" height="4"/></svg>';
        const out = sanitizeSvg(input, { idPrefix: 'nl-logo' });
        expect(out).toMatch(/url\(#nl-logo-\d+\)/);
    });

    it('is deterministic for identical input', () => {
        const input = '<svg><g id="a"><rect x="0" y="0" width="4" height="4"/></g><use href="#a"/></svg>';
