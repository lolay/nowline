...
describe('CAPACITY_ICON_SVG library', () => {
    ...
    it('exposes exactly the four built-in SVG glyphs', () => {
        expect(Object.keys(CAPACITY_ICON_SVG).sort()).toEqual(
    ...
    it.each(expectedNames)(
        '%s body uses currentColor so renderer can recolor inline',
        (name) => {
    ...
    it.each(expectedNames)(
        '%s body is a non-empty string of SVG primitives',
        (name) => {
