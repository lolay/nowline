...
| `export` | `{ source?: string, path?: string, format: 'svg' \| 'pdf' \| 'html' \| 'mermaid' \| 'xlsx' \| 'msproj' \| 'png', delivery?: 'file' \| 'inline' \| 'both', ... }` | file resource + metadata | Same `@nowline/export` kernel and format adapters as `nowline <input> -f <format>`, run in-process. Byte-identical to the CLI for the same source + inputs ([export-determinism](./export-determinism.md)). `svg` is accepted here (in addition to `render`) so agents can save a vector file via `delivery:"file"`. The optional `delivery` parameter controls file vs. inline delivery (see [Structured output](#structured-output) § export delivery). |
...
**User-configurable output folder.** The manifest declares a `user_config.output_dir` directory picker (default `${HOME}/Downloads`). Claude Desktop prompts the user to select a folder on first install and injects it as `--root <dir>` in the server arguments. This makes `--root` govern both the export destination and the read/create/list/delete sandbox for the bundle — users can re-point the picker at their roadmaps folder. With a root configured, the smart export default writes `pdf`/`xlsx` files to disk instead of returning inline bytes (which Claude Desktop drops silently). Manifest version bumped to `0.4` (adds `user_config` support).

**Root token expansion.** Claude Desktop's mcpb variable substitution is single-pass: when the user installs without opening the directory picker, the optional `output_dir` default reaches the server with `${HOME}` *unexpanded* (literal `${HOME}/Downloads`). To stay robust, [`packages/mcp/src/root-path.ts`](../packages/mcp/src/root-path.ts) expands the mcpb path tokens (`${HOME}`, `${DESKTOP}`, `${DOCUMENTS}`, `${DOWNLOADS}`, `${pathSeparator}`/`${/}`), a leading `~`, and generic `${NAME}`/`$NAME` environment variables before constructing the server, resolving the result to an absolute path. A blank value (optional field left unset, passed as `""`) collapses to `undefined` so the server falls back to cwd with inline delivery rather than a bogus root.
