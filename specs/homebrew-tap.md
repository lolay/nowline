...
- **Code-sign + notarize macOS binaries.** Tracked as [`specs/milestones.md`](./milestones.md) § m4.10. Lolay already holds an Apple Developer account, so the $99/year Developer ID requirement is met — this is deferred on priority, not cost. Eliminates Gatekeeper friction for direct GitHub Release downloads; Homebrew installs already work without it. Pairs naturally with the "tarball each binary" item above, since a bare binary (not a `.app`/`.pkg`/archive) can't be stapled directly.
