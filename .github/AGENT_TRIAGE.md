...
- **Copilot session opened a PR but `agent-review.md` didn't fire.** The review phase triggers on `pull_request.opened`/`synchronize` for PRs by `copilot-swe-agent[bot]` specifically. If Copilot's bot identity changes upstream, this trigger needs an update. Check `gh run list --workflow=agent-review.md --limit 5` and the PR's author.
