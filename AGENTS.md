...
- **`make pre-commit` is non-negotiable**: if it fails for any environmental reason (pnpm TTY error, missing binary, stale staging dir), fix the environment first — do not substitute a partial manual check (e.g. running biome directly on a subset of files). A partial check is silent about everything it didn't cover. The correct sequence is always `make pre-commit` then `make pack-mcpb`; `pack-mcpb` needs the `dist/` that `pre-commit → build` produces anyway.
