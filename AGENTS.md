...
Everything goes through the [`Makefile`](./Makefile) — it is the single source of truth for build / test / lint / package / publish commands, and CI calls the same targets, so a green `make pre-commit` locally (alias of `make ci`) is the gate CI runs. Run `make help` for the grouped target list; [`Makefile.md`](./Makefile.md) is the narrative reference.
...
- **PRs**: one logical change each, squash-merged. Run `make pre-commit` locally before committing or pushing — it chains lint + typecheck + build + test, the same gate CI runs as `make ci` on Linux, macOS, and Windows. (`make lint` is biome lint + format-drift + import order; skipping it is the failure mode that lets formatting regressions slip into a `bd59e70`-style dep bump.)
...
- **`make pre-commit` is non-negotiable**: if it fails for any environmental reason (pnpm TTY error, missing binary, stale staging dir), fix the environment first — do not substitute a partial manual check (e.g. running biome directly on a subset of files, or using the IDE `ReadLints` tool). A partial check is silent about everything it didn't cover. The correct sequence is always `make pre-commit` then `make pack-mcpb`; `pack-mcpb` needs the `dist/` that `pre-commit → build` produces anyway.
- **Re-run `make pre-commit` immediately before staging and committing**: if any file was edited after the last green `make pre-commit` run — including by a review pass, a fix-up, or a multi-turn agent session — run `make pre-commit` again before `git add`. A green run from an earlier turn does not cover later edits. `ReadLints` is not a substitute: it only invokes the IDE TypeScript language server and is blind to Biome format and lint rules entirely.
- **Bug retrospective**: when you fix a bug that escaped to CI, a workflow run, or production, ask why `make pre-commit` didn't catch it. If a check could reasonably have caught it (a missing lint rule, an uncovered test path, a type hole), add or tighten that check in the same PR so the gate catches the class of bug next time.
- **Docs**: if observable behavior changes, update the relevant spec under [`specs/`](./specs/) and any affected `README.md` in the same PR. For a full doc audit (human + agent consumption), use the [`optimize-docs` skill](.cursor/skills/optimize-docs/SKILL.md).
